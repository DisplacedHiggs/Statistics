...
while read model; do
  ...
  echo "*******************    run fit        ***********************"
  echo "*************************************************************"
  ./MakeWorkspace_VR_SYS_ALL_TF_EMuCombo --signal_model=$model
  echo "*************************************************************"
  echo "*******************    copy to outdir        ****************"
  echo "*************************************************************"
  ...
  combine -M MultiDimFit card_systematics.root  --saveWorkspace --setParameters mask_TwoMuZH=1,r=0 -n MaskedDataSnapshot --freezeParameters r
  echo "*************************************************************"
  echo "*******************    run combine       ********************"
  echo "*************************************************************"
  #combine -M AsymptoticLimits card_systematics.root --run blind
  combine -M AsymptoticLimits higgsCombineMaskedDataSnapshot.MultiDimFit.mH120.root  --saveWorkspace --setParameters mask_TwoMuZH=0 --bypassFrequentistFit --snapshotName MultiDimFit -t -1 --toysFrequentist  -n NoMaskAsimovFromMaskedDataSnapshot
  ls $output_dir/$model
  cd -
